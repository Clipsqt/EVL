...
$insertQuery = "INSERT INTO e_logsHistory (fullname, sex, priority, phonenumber, scheduledate, position_designation, agency_school_office, appointment, purpose_of_visit, department, reference_no, time_in, time_out, assisted_by) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = mysqli_prepare($conn, $insertQuery);
mysqli_stmt_bind_param($stmt, "ssssssssssssss", $row['fullname'], $row['sex'], $row['priority'], $row['phonenumber'], $row['scheduledate'], $row['position_designation'], $row['agency_school_office'], $row['appointment'], $row['purpose_of_visit'], $row['department'], $row['reference_no'], $row['time_in'], $row['time_out'], $_SESSION['accountName']);
