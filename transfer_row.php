...
$insertQuery = "INSERT INTO e_logsHistory (fullname, sex, priority, phonenumber, scheduledate, appointment, purpose_of_visit, department, reference_no, time_in, time_out, assisted_by) 
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = mysqli_prepare($conn, $insertQuery);
mysqli_stmt_bind_param($stmt, "ssssssssssss", $row['fullname'], $row['sex'], $row['priority'], $row['phonenumber'], $row['scheduledate'], $row['appointment'], $row['purpose_of_visit'], $row['department'], $row['reference_no'], $row['time_in'], $row['time_out'], $_SESSION['accountName']);
